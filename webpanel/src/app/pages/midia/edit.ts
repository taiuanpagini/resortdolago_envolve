...
declare var $: any;
@Component({
  selector: 'app-midia-edit',
  templateUrl: './edit.html'
})
export class MidiaEditComponent implements OnInit {
  ...
  *************/
  getItem() {
    this.ActivatedRoute.params.subscribe(params => {
      ...
      this.HttpService.JSON_GET(`/data/item/midias/${params.id}/*/titulo`, headerConfig)
        .then(
        (res) => {
  ...
  putItem(form) {
    this.ActivatedRoute.params.subscribe(params => {
      if (form.status === 'INVALID') {
        ...
        this.HttpService.JSON_PUT(`/data/item/midias/${params.id}/*/tipo`, headerConfig, form.value)//CADASTRO DE midiaS      
          .then(
          (res) => {
            
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
            .then(() => { this.Router.navigate(['/midia/lista']); })
