  ...
  *************/
  getItem() {
    this.ActivatedRoute.params.subscribe(params => {
      ...
      this.HttpService.JSON_GET(`/data/item/contatos/${params.id}/*/titulo`, headerConfig)
        .then(
        (res) => {
  ...
  putItem(form) {
    this.ActivatedRoute.params.subscribe(params => {
      if (form.status === 'INVALID') {
        ...
        this.HttpService.JSON_PUT(`/data/item/contatos/${params.id}/*/titulo`, headerConfig, form.value)//CADASTRO DE contatoS      
          .then(
          (res) => {
            
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
            .then(() => { this.Router.navigate(['/contato/lista']); })
