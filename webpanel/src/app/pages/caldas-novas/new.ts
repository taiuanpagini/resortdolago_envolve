...
declare var $: any;
@Component({
  selector: 'app-caldas-new',
  templateUrl: './new.html'
})
export class CaldasNewComponent implements OnInit {
  ...
  *************/
  postItem(form) {
    ...
    if (form.status === 'INVALID') {
      ...
      this.HttpService.JSON_UPLOAD_POST(`/data/caldas/titulo`, headerConfig, form.value, true)//CADASTRO DE BANNERS
        ...
        .then(
          (res) => {
            ...
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
              .then(() => { 
                this.Router.navigate([`caldas-novas/${res.json().data[0].hashID}/upload-imagem`]); 
