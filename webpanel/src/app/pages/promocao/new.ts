  ...
  *************/
  postItem(form) {
    ...
    if (form.status === 'INVALID') {
      ...
      this.HttpService.JSON_UPLOAD_POST(`/data/promocoes/titulo`, headerConfig, form.value, true)//CADASTRO DE BANNERS
        ...
        .then(
          (res) => {
            ...
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
              .then(() => { 
                this.Router.navigate([`promocao/${res.json().data[0].hashID}/upload-imagem`]); 
