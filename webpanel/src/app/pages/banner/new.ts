  ...
  *************/
  postItem(form) {
    ...
    if (form.status === 'INVALID') {
      ...
      this.HttpService.JSON_UPLOAD_POST(`/data/banners/titulo`, headerConfig, form.value, true)//CADASTRO DE BANNERS
        ...
        .then(
          (res) => {
            ...
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
              .then(() => { 
                this.Router.navigate([`banner/${res.json().data[0].hashID}/upload-imagem`]); 
