...
declare var $: any;
@Component({
  selector: 'app-resort-new',
  templateUrl: './new.html'
})
export class ResortNewComponent implements OnInit {
  ...
  *************/
  postItem(form) {
    ...
    if (form.status === 'INVALID') {
      ...
      this.HttpService.JSON_UPLOAD_POST(`/data/resort/titulo`, headerConfig, form.value, true)//CADASTRO DE BANNERS
        ...
        .then(
          (res) => {
            ...
            Promise.resolve(this.AlertService.alertInfo(res.json().msg))
              .then(() => { 
                this.Router.navigate([`resort/${res.json().data[0].hashID}/upload-imagem`]); 
